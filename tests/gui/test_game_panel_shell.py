...
def test_export_threads_ds_main_story_scope(qtbot, tmp_path, monkeypatch):
    ...
    with open(pl, "w", newline="", encoding="utf-8") as f:
        w_csv = csv.DictWriter(f, fieldnames=PLAYLIST_COLUMNS)
        w_csv.writeheader()
        w_csv.writerow(dict(episode="0", is_side="0", pos="0.0", section="0", scene="s",
                            line_index="0", track_index="0", category="cutscene", speaker="Sam",
                            subtitle="Hi.", stream_path="loc/a.wem.english.core.stream",
                            line_id="a"))
