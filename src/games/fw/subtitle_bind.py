...
def scan_arith_clean_groups(graph, reader, store, transcripts_by_id,
    ...
    from engine.pack.fw_object_reader import read_group_spans

    LSSR = type_hash("LocalizedSimpleSoundResource")
    LANGS = 12
    if en_indices is None:
        from engine.pack.fw_fast_extract import english_file_indices
...
def main(argv=None):  # pragma: no cover - integration glue
    ...
    from engine.pack.fw_rtti import TypeRegistry
    from engine.pack.fw_object_reader import GroupReader
